...
Pointer.prototype = {
	...
	setDOMEvents: function () {
		...
		this._events = events = [
			...
			[container, 'ondblclick', 'onContainerDblClick'],
		...
		input.onchange = function () {
			...
			if (isNaN(value)) {
				...
				*/
				...
				// 插件原来的不支持有时分秒的时间转换
				var aValue = inputValue.split(' ');
				var aValue_date = aValue[0].split('-');
				var aValue_time = aValue[1].split(':');
				value = inputValue.split('-');
				value = Date.UTC(pInt(aValue_date[0]), pInt(aValue_date[1]) - 1, pInt(aValue_date[2]), pInt(aValue_time[0]), pInt(aValue_time[1]));
